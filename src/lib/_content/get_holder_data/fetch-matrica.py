...
# with open('C:\\Users\\33607\\Downloads\\Chimpions.co\\src\\lib\\_content\\matrica_data.json', 'r') as f:
#     holders = json.load(f)

# for holder_wallet in file.readlines():
#     holder_wallet = holder_wallet[:-1]
#     if holder_wallet not in holder_wallets:
#         holder_wallets.append(holder_wallet)
#         username, pfp = get_username_and_pfp(holder_wallet)
#         url = f"https://api.matrica.io/v1/wallet/{holder_wallet}?apiKey={matrica_api_key}"
#         data = make_api_call(url)
#         holders[username]["pfp"] = data["user"]["profile"]["pfp"]

# with open('matrica_data1.json', 'w') as f:
...
def main():
            ...
            if nft["collection"] is not None:
                ...
                if collection_id in collection_ids.keys():
                    ...
                    if collection_id == "12166093-97ad-44b5-9d74-a5cbabbeb8a6":
                        holders[holder_name]["chimpions"].append(nft["name"])
                if collection_id in TWS_ids.keys():
                    holders[holder_name]["TWS"][TWS_ids[collection_id]] = True
        # if len(holders[holder_name]["chimpion"]) == 0:
        #     holders.pop(holder_name)
    ...
    with open("./matrica_data.json", 'w') as f:
        json.dump(holders, f, indent = 4)
